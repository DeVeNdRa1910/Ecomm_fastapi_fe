...
import { IndianRupee, Package, ShoppingCart, Tag, ArrowLeft, Loader2 } from "lucide-react"
...
export default function CategoryProductsPage() {
  ...
  const fetchProducts = async () => {
    setIsLoading(true)
    try {
      const allProducts = await productApi.getProductsByCategory(category)
  ...
  const categoryLabel = CATEGORY_LABELS[category] || formatCategory(category)
            ...
            >
              ...
              <div className="text-center">
                ...
                <p className="text-muted-foreground text-lg">
                  {products.length > 0 ? `Found ${products.length} product${products.length !== 1 ? 's' : ''}` : 'Browse products in this category'}
                </p>
              </div>
            </motion.div>
